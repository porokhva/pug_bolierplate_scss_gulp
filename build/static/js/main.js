...
document.addEventListener("DOMContentLoaded", function () {
  ...
  }(window.Element.prototype); // мультиэвентлисэнер
  ...
  (() => {
    var isSafari;
    window.navigator.vendor.toLowerCase().indexOf("apple") > -1 ? isSafari = true : isSafari = false;
  ...
  (() => {
    var walletBtn = document.body.querySelector(".js_wallet_show");
    var walletBody = document.body.querySelector(".js_account_wallet");
    var hasWallet = document.body.querySelector("header").contains(walletBtn);
    ...
    if (hasWallet) {
      ...
      document.body.addEventListener("click", e => {
        var target = e.target;
        target == walletBtn || target.closest(".js_wallet_show") ? toggle(e) : target !== walletBody ? remove() : false;
      });
    }
  })();
  ...
  if (document.body.classList.contains("js-pawnshop-main")) {
    ...
    function Modal1() {
      var openButtons = document.body.querySelectorAll(".js_modal_open"),
          overlay = document.body.querySelector(".js_overlayed_bg"),
          closeButtons = document.body.querySelectorAll(".js_modal_button_close"),
          body = document.querySelector("body"); // для тела модалки и конпки прописан дата-атрибут (data-modal=""),
      ...
      openButtons.forEach(function (item) {
        item.addEventListener("click", function (e) {
          ...
          var modalId = this.getAttribute("data-modal"),
              modalInnerItem = document.body.querySelector(".js_modal_copy_cloned"),
              modalElem = document.body.querySelector('.js_modal[data-modal="' + modalId + '"]'),
              addElemCopy = this.closest(".inner").children; // очищаем модалку при открытии

          modalInnerItem.innerHTML = "";

          for (var i = 0; i < addElemCopy.length; i++) {
            var el = addElemCopy[i];

            if (el.classList.contains("js_modal_copy")) {
              // копируем объявление
              var clonedInnerCard = el.cloneNode(true); //удаляем описание

              for (var _i = 0; _i < clonedInnerCard.children.length; _i++) {
                var _el = clonedInnerCard.children[_i];

                if (_el.classList.contains("descr")) {
                  _el.remove();
                }
              } // вставляем в модалку
      ...
      document.body.addEventListener("keyup", function (e) {
        ...
        if (key == 27) {
          document.body.querySelector(".js_modal.active").classList.remove("active");
          overlay.classList.remove("active");
          body.classList.remove("no-scroll");
        }
      }, false);
  ...
  jsTriggers.forEach(function (trigger) {
    trigger.addEventListener("click", function () {
      var id = this.getAttribute("data-tab"),
          content = document.body.querySelector('.js-tab-content[data-tab="' + id + '"]'),
          activeTrigger = document.body.querySelector(".js-tab-trigger.active"),
          activeContent = document.body.querySelector(".js-tab-content.active");
...
function imageLoading(container_loaded, max) {
  var fileInput = document.getElementById("file-input"),
      //  инпут с файлами
  delBtn = document.body.querySelector(".del-info"),
      // кнопка удаления (блок)
  ...
  function clearInput() {
    var loadedImg = document.body.querySelectorAll(".".concat(container_loaded));

    if (loadedImg.length > 0) {
      for (var i = 0; i < loadedImg.length; i++) {
        loadedImg[i].remove();
      }
  ...
  function handleFileSelect(e) {
    ...
    for (var _i2 = 0; _i2 < loadedImg.length; _i2++) {
      loadedImg[_i2].remove();
    }
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if (!f.type.match("image.*") || !f.type.startsWith("image/")) {
        ...
        showModal("\u0422\u043E\u043B\u044C\u043A\u043E \u0438\u0437\u043E\u0431\u0440\u0430\u0436\u0435\u043D\u0438\u044F \u0432 \u0440\u0430\u0437\u043C\u0435\u0440\u0435 \u043D\u0435 \u0431\u043E\u043B\u0435\u0435 500\u043A\u0431", "error");
      } else if (files.length > max) {
        showModal("\u041D\u0435 \u0431\u043E\u043B\u044C\u0448\u0435 ".concat(max, " \u0438\u0437\u043E\u0431\u0440\u0430\u0436\u0435\u043D\u0438\u0439"), "error");
      } else {
        ...
        reader.addEventListener("load", function (e) {
          ...
          imageBox.innerHTML = "<img class=\"thumb\" title=\"".concat(pic.result.name, "\" src=\"").concat(pic.result, "\"/>");
...
function Modal() {
  ...
  document.body.querySelectorAll(".js_switch_modal").forEach(function (item) {
    ...
    item.addEventListener(event, function (e) {
      e.preventDefault();
      var currentModal = this.closest(".js_modal").getAttribute("data-modal"),
          currentModalEl = document.body.querySelector('.js_modal[data-modal="' + currentModal + '"]'),
          nextModal = this.getAttribute("data-next-modal"),
          nextModalEl = document.body.querySelector('.js_modal[data-modal="' + nextModal + '"]');
...
function seeMore() {
  var showButtons = document.body.querySelectorAll(".js_button_seemore");
  ...
  if (showButtons !== null || showButtons !== undefined) {
    showButtons.forEach(button => {
      ...
      button.addEventListener("click", e => {
        e.preventDefault();
        var showBox = e.target.nextElementSibling;
        var buttonArrow = e.target.lastChild; // e.target.closest('.js_replays_box')
...
function openMenu(button, boxes) {
  ...
  openButton.addEventListener("click", e => {
    var showBox = document.body.querySelectorAll(boxes);
    e.preventDefault();
...
function pagePagination(item, paginator) {
  ...
  if (item !== undefined && item !== null && paginator !== undefined && paginator !== null) {
    for (var i = 0; i < pagesNumber; i++) {
      page += "<span data-page=" + i * maxElements + '  id="page' + (i + 1) + '">' + (i + 1) + "</span>";
    }

    paginator.innerHTML = page; //выводим первые записи {maxElements}
    ...
    function _pagination(event) {
      ...
      for (var i = 0; i < div_num.length; i++) {
        ...
        if (data_num <= data_page || data_num >= data_page) div_num[i].style.display = "none";
      }
...
function hideLabel() {
  var inputs = document.body.querySelectorAll(".js_input_hide_label");
...
function cropText(box, numberSymbols) {
  var cropElement = document.body.querySelectorAll(box),
      // выбор элементов
  size = numberSymbols,
      // кол-во символов
...
function scrollToTop() {
  var arrowTop = document.body.querySelector("#arrow_top"); // скрываем на первом экране
  ...
  arrowTop.addEventListener("click", function () {
    scrolled = window.pageYOffset; //window.scrollTo(0,0);
...
var inputsDigit = document.querySelectorAll(".input_digit");

if (inputsDigit.length > 0 && inputsDigit !== null && inputsDigit !== undefined) {
...
function formatInputs(inputs) {
  inputs.forEach(input => {
    ...
    input.addEventListener("input", () => {
      // только одна точка
      input.value = input.value.replace(/(^[^.]*.)|[.]+/g, "$1"); // и без пробелов

      input.value = input.value.replace(/\s/g, ""); // и только цифры
